...
- 管理后台（`pc/`，桌面浏览器设计）：
  ...
  - 报名管理（查看全部报名状态/筛选/搜索/编辑/导出 CSV/详情）
  - 一面管理（标记面试/一面签到号/通过/不通过/撤销/导出 CSV）
  - 二面管理（标记面试/签到号/通过/不通过/批量）
  - 录取名单（待确认/已录取/部门筛选/提醒确认）
  - 通知管理（发送：全部/按状态/指定用户；历史编辑/删除）
  - 系统设置（独立报名截止与录取确认截止、一面二面时间地点与签到开关、部门详情）

## 状态流转

```
报名提交 → registered(报名成功)
  ├─ 公布一面时间 → waiting_first(等待一面)
  │    ├─ 一面通过(+部门) → first_passed
  │    │    ├─ 公布二面时间 → waiting_second(等待二面)
